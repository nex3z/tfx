...
def parse_model_path(
  ...
  if not version or not version.isdigit():
    raise ValueError('Version segment does not exist.')
  if not rest:
    raise ValueError('Model path is too short.')
  model_base_path, model_name = os.path.split(rest)
  if expected_model_name is not None and expected_model_name != model_name:
    raise ValueError('model_name ({}) != expected_model_name ({})'.format(
        model_name, expected_model_name))

  return model_base_path, model_name, int(version)
